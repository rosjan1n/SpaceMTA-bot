...
const { Events, ButtonBuilder, ButtonStyle, ActionRowBuilder, PermissionsBitField, ChannelType } = require("discord.js");
const { verifyRoleId, ticketWhitelistRolesId } = require("../config/config.json")
const fs = require("fs");
  ...
  async execute(interaction) {
    if (interaction.isCommand()) {
        ...
        const res = new EmbedBuilder()
          ...
          try {
            ...
            res
              .setTitle(`Ticket`)
              .setColor(5763719)
              .setDescription(`Panel został wysłany na kanał <#${selectedChannel.id}>.`)
              .setTimestamp()
              .setFooter({
                text: "SpaceMTA - Ticket",
                iconURL: 
      ...
      if (interaction.customId === "verifyButton") {
        ...
        var ticketCategory = guild.channels.cache.find((c) => c.type == ChannelType.GuildCategory && c.name.toLowerCase() == "tickety")
        const ticketExist = ticketsData.find((ticket) => ticket.creatorId === user.id)

        if (ticketExist) {
          const res = new EmbedBuilder()
            .setColor(15548997)
            .setTitle("Wystąpił błąd")
            .setDescription(`Masz już otwarty ticket <#${ticketExist.channelId}>`)
            .setTimestamp()
            .setFooter({
              text: "SpaceMTA - Ticket",
              iconURL: 
        ...
        try {
          const ticketId = (ticketsData.length > 0 ? ticketsData[ticketsData.length - 1].id + 1 : 1).toString().padStart(3, "0");
          const newChannel = await guild.channels.create({ name: `ticket-${ticketId}` })

          await newChannel.setParent(ticketCategory.id)

          await newChannel.permissionOverwrites.edit(user, { ViewChannel: true, SendMessages: true });

          ticketWhitelistRolesId.forEach(async (roleId) => {
            const role = guild.roles.cache.find((role) => role.id == roleId)
            if (role) {
              await newChannel.permissionOverwrites.edit(role, { ViewChannel: true, SendMessages: true })
            }
          })
          ...
          const res = new EmbedBuilder()
            ...
            .setColor(5763719)
            .setDescription(`Twój ticket został pomyślnie stworzony. <#${newChannel.id}>`)
            .setTimestamp()
            .setFooter({
              text: "SpaceMTA - Ticket",
              iconURL: 
          ...
          res
            .setTitle(`Ticket #${ticketId}`)
            .setColor(0x0099ff)
            .setDescription("Opisz swój problem, następnie cierpliwie oczekuj na odpowiedź od Naszego zespołu.\nPamiętaj, aby nikogo nie oznaczać.")
            .setTimestamp()
            .setFooter({
              text: "SpaceMTA - Ticket",
              iconURL: 
          ...
          await newChannel.send({ content: `<@${user.id}>`, embeds: [res], components: [row] })

          ticketsData.push({ id: ticketsData.length > 0 ? ticketsData[ticketsData.length - 1].id + 1 : 1, creatorId: user.id, channelId: newChannel.id })
        ...
        const ticket = ticketsData.find((ticket) => ticket.channelId == channelId);
        const res = new EmbedBuilder()
          .setColor(15548997)
          .setTimestamp()
          .setFooter({
        ...
        try {
          ...
          res
            .setDescription("Ticket został zamknięty.")
            .setAuthor({ 
          ...
          const creator = guild.members.cache.find((user) => user.id === ticket.creatorId);

          channel.edit({ name: `closed-${ticket.id.toString().padStart(3, "0")}`})
